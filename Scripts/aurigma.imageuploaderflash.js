...
getGlobalHandlerName = function (uploader, eventName) {
	return handlerNames[uploader.id() + '_' + eventName] ||
		(handlerNames[uploader.id() + '_' + eventName] = 
			'__' + eventName + '_' + createGUID().replace(/-/g, ''));
},
...
getParams = function (type) {
    ...
    if (this._simpleProperties && this._simpleProperties.length > 0) {
        for (i = 0, cnt = this._simpleProperties.length; i < cnt; i++) {
            if (this._simpleProperties.hasOwnProperty(i) && !this._simpleProperties[i].isAttribute || type === 'html') {
...
var browser = AU.browser = new (function () {
	...
	this.isMobile = a.indexOf("iphone") > -1 || a.indexOf("ipad") > -1 || a.indexOf("android") > -1;
	this.isHtmlUploaderSupported = function () {
		return AU.Control && AU.Control.isHtmlUploaderSupported();
	};
	this.isThumbnailConverterSupported = function () {
		return AU.Control && AU.Control.isThumbnailCreationSupported();
	};
	this.isImageRestrictionsSupported = function () {
		return AU.Control && AU.Control.isImageRestrictionsSupported();
	};
})();
var propertyMaker = {
    ...
    createControlProperty: function (obj, property) {
        ...
        if (typeof property.name === 'string' && !property.renderOnly) {
            try {
            	var field = '_' + property.name,
            		// save initial property value
					oldValue = obj[field],
                    uploader = obj._uploader,
                    type = uploader.type(),
                    control = uploader.getControl();
                ...
                // Can not perform this check for flash uploader
                if (type === 'html') {
                    if (getter && control[getter] == null) {
                        showInfo('"' + getter + '" is undefined.');
                    }

                    if (setter && control[setter] == null) {
                        showInfo('"' + setter + '" is undefined.');
                }

                obj[property.name] = function () {
                    // set value if we pass parameter
                    if (arguments.length > 0) {
                            return control[setter](arguments[0]);
                        } else {
                            return control[getter](propName);
                        }
                    };
                } else if (type === 'flash') {
                    obj[property.name] = function () {
                        // set value if we pass parameter
                        if (arguments.length > 0) {
                            return control.callFlash(setter, arguments[0]);
                    } else {
                            return control.callFlash(getter);
                    }
                };
                } else {
    ...
    createAttributeProperty: function (obj, property) {
        ...
        ///	    The object which contains method name and defaut value.
        ///	</param>
        var uploader = obj._uploader;
        if (uploader.type() === 'html') {
            propertyMaker.createControlProperty(obj, property);
        } else if (uploader.type() === 'flash') {
            var control = uploader.getControl();
        obj[property.name] = function () {
            // set value if we pass parameter
            if (arguments.length > 0) {
                return control[property.name] = arguments[0];
            } else {
                //return value otherwise
                return control[property.name];
            }
        };
        }
    },
    createMethod: function (obj, method) {
        ...
        if (type === 'html') {
            obj[method.name] = function () {
                return control[controlMethodName].apply(control, arguments);
            };
        } else if (type === 'flash') {
        obj[method.name] = function () {
            ...
            switch (arg.length) {
                case 0:
                    return control.callFlash(controlMethodName);
                case 1:
                    return control.callFlash(controlMethodName, arg[0]);
                case 2:
                    return control.callFlash(controlMethodName, arg[0], arg[1]);
                case 3:
                    return control.callFlash(controlMethodName, arg[0], arg[1], arg[2]);
                case 4:
                    return control.callFlash(controlMethodName, arg[0], arg[1], arg[2], arg[3]);
                case 5:
                    return control.callFlash(controlMethodName, arg[0], arg[1], arg[2], arg[3], arg[4]);
            }
        };
        } else {
            throw new Error('Not supported: ' + type);
    ...
    _typicalInit: function (obj, uploader) {
        ...
        if (obj._simpleProperties) {
            for (var i = 0, imax = obj._simpleProperties.length; i < imax; i++) {
                 if(obj._simpleProperties.hasOwnProperty(i)){
        ...
        if (obj._objectProperties) {
            for (var i = 0, imax = obj._objectProperties.length; i < imax; i++) {
               if(obj._objectProperties.hasOwnProperty(i)){
...
AU.flashDetector = {
    detectVersion: function() {
        ...
        if (plugins && typeof ((plg = plugins["Shockwave Flash 2.0"] || plugins["Shockwave Flash"])) !== 'undefined') {
            ...
            var matches = /^.+\s+(\d+).(\d+)\s+\w(\d+)$/.exec(flashDescr);
...
AU.debug = function () {
    ...
    if (!AU.debug._instance) {
        var f = function () {
            this.constructor = AU.debug;
        };
        f.prototype = AU.debug.prototype;
        AU.debug._instance = new f();
    }
    return AU.debug._instance;
};
...
AU.debug.prototype = {
    ...
    version: function () {
      return AU.Control.Uploader.prototype.getActualVersion();
    },
    buildDate: function () {
      return AU.Control.Uploader.prototype.getActualReleaseDate();
    },
    mode: function (mode) {
        ///	<summary>
    ...
    ///	<summary>
    ///		Show message from Aurigma Upload Suite.
    ///     The method called from Aurigma Upload Suite trace event.
    ///	</summary>
    ///	<param name="msg" type="String">
    ///     Messsage
    ///	</param>
            ...
            switch (this._mode[i]) {
                ...
                case 'popup':
                    if (!this._popupWindow || this._popupWindow.closed) {
                        // open popup window
                        this._popupWindow = window.open('', 'UploadSuiteDebugWindow',
                        ...
                        if (!list || list.length == 0) {
                            ...
                            html.push('<head>');
                            html.push('<title>Aurigma Upload Suite Log Window</title>');
...
function baseControl() {
	...
	this._simpleProperties = [{ name: "id", getter: 1, setter: 1, isAttribute: true}];
}
...
baseControl.prototype = {
	...
	type: function () {
		function detectType(typeString) {
			...
			for (var i = 0, imax = types.length - 1; i < imax; i++) {
				...
				if (type === 'html') {
					// Can we use html5?
					if (browser.isHtmlUploaderSupported()) {
						var supported = true;
						if (supported && !browser.isImageRestrictionsSupported()) {
							var restrictions = this.restrictions();
							supported = !(restrictions.minImageWidth() > 0 || restrictions.minImageHeight() > 0 ||
									restrictions.maxImageWidth() > 0 || restrictions.maxImageHeight() > 0);
						}
						if (supported && !browser.isThumbnailConverterSupported()) {
							var converters = this.converters();
							for (var j = 0, jmax = converters.count(); j < jmax; j++) {
								var converter = converters.get(j);
								if (converter && /=thumbnail/i.test(converter.mode())) {
									supported = false;
									break;
								}
							}
	...
	state: function () {
        ...
        amazonS3Extender: function () {
		if (arguments.length > 0) {
			this._amazonS3Extender = arguments[0];
		} else {
			return this._amazonS3Extender;
		}
	},
	...
	render: function (parentElement) {
				...
				if (event._handlers && event._handlers.length > 0) {
					...
					switch (eventName) {
						case 'restrictionFailed':
							args = [e.code, e.fileName, e.message];
							break;
                                                case 'beforePackageUpload':
							args = [e.index];
							break;
						case 'beforeSendRequest':
							args = [e.index, e.fileName, e.packageGuid, e.fileGuid];
							break;
						case 'afterPackageUpload':
							args = [e.index, e.response];
							break;
						case 'afterUpload':
							args = [e.response];
							break;
						case 'error':
							args = [e.errorCode, e.httpStatus, e.response, e.additionalInfo, e.request];
							break;
						case 'progress':
							args = [e.percent, e.uploadedFiles, e.totalFiles, e.uploadedBytes, e.totalBytes];
							break;
						case 'trace':
							args = [e.log];
							break;
							...
							args = [e.select];
							break;
						case 'imageEditorClose':
							args = [e.index,e.result,e.guid];
							break;
					}
		...
		var paramsArr = this.getParams();
		var o = {};
                var val;
		for (var i = 0, imax = paramsArr.length; i < imax; ++i) {
			var paramName = paramsArr[i].name;
			if (/listener$/.test(paramName)) {
				continue;
			}
			paramName = paramName.substring(0, 1).toUpperCase() + paramName.substring(1);
			val = paramsArr[i].value;
                        if(typeof val == "string" && "true|false".indexOf(val) > -1){
                           paramsArr[i].value =  val == "true";
                        }

			o[paramName] = paramsArr[i].value;
		}

		var control = this._htmlUploaderControl = new AU.Control.Uploader(o);
		...
		for (var i in events) {
			if (i != 'getParams' && i != 'preRender') {
				...
				if (typeof event === 'function' && event() instanceof AU.event && event().count() > 0) {
					control.addEventListener(i, createEventHandler(event(), i));
				}
			}
		}
	...
	writeHtml: function (parentElement) {
		var type = this.type();
		if (type === 'html') {
			var placeholder;
			if (!parentElement) {
				placeholder = 'au_uploader_placeholder' + ((Math.random() * 1000000) | 0);
				document.write('<div style="display: none;" id="' + placeholder + '"></div>');
			}
			this.render(parentElement);
			if (placeholder) {
				placeholder = document.getElementById(placeholder);
				var uploader = document.getElementById(this.id());
				placeholder.parentNode.replaceChild(uploader, placeholder);
			}
		} else if (type === 'flash') {
			window.document.write(this._getFlashHtml());
		} else {
			throw new Error('Not supported: ' + type);
		}
	},
	...
	_getFlashHtml: function () {	
		if (!this.flashControl().isFlashSupported() && browser.isMobile) {
		    var html = [];
		    ...
		    html.push('<div style="position: relative; z-index: 1000; background-color: #f7f7f7; overflow: hidden; width: '
                    + w + '; height: ' + h + '">');
		    html.push('<div style="text-align:center; margin-top: 10px;"> Browser is not supported. </div>');
		    html.push('</div>');
					...
					for (var i = 0, imax = event._handlers.length; i < imax; i++) {
						try {
							...
							if (typeof handler === 'function') {
								...
								result = window[handler].apply(self, arguments);
							} else {
								showError(eventName + " error:\n \"" + handler + "\" handler is not defined.");
							}
							if (eventName === 'beforeUpload' || eventName === 'afterPackageUpload') {
	...
	_getHtmlHtml: function () {
		var self = this;
		var suffix = (Math.random() * 100000) | 0;
		// Add random suffix to make sure id and function is unique
		var id = '__' + self.id() + '_initElement' + suffix;
		var func = ('__' + self.id() + '_initFunction' + suffix).replace(/\W+/g,"");
		...
		// Return simple html wich trigger actual rendering code
		// Image has src property wich can not be loaded and 
		// therefor trigger error event handler, which is the actual 
		// render function.
		return  '<div id="' + id + '"></div><img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" onload="' + func + '();" />';
	},
	...
	getControl: function () {
		var type = this.type();
		if (type === 'html') {
			return this._htmlUploaderControl;
		} else if (type === 'flash') {
	...
	getParams: function () {
		var params = getParams.call(this, this.type());
...
AU.converter.prototype = {
    ...
    _simpleProperties: [
        ...
        { name: "thumbnailCopyColorProfile", getter: 1, setter: 1, render: "ConverterThumbnailCopyColorProfile" },
        ...
        { name: "thumbnailApplyDrawTool", getter: 1, setter: 1, render: "ConverterThumbnailApplyDrawTool" },
        { name: "autoReduceDimensions", getter: 1, setter: 1, render: "ConverterAutoReduceDimensions" },
        { name: "hash", getter: 1, setter: 1, render: "ConverterHash" },
        { name: "getThumbnailWatermarkPreview", render: "getConverterThumbnailWatermarkPreview" },
        { name: "thumbnailResizeFailureDetector", getter: 1, setter: 1, render: "ConverterCanvasResizeAttempt" },
        { name: "thumbnailSizeLimit", getter: 1, setter: 1, render: "ConverterThumbnailSizeLimit" },
        { name: "thumbnailType", getter: 1, setter: 1, render: "ConverterThumbnailType" },
        { name: "thumbnailUseExifPreview", getter: 1, setter: 1, render: "ConverterUseExifPreview" },
        { name: "thumbnailResizeCmyk", getter: 1, setter: 1, render: "ConverterThumbnailResizeCmyk" },
    ...
    thumbnailCopyColorProfile: function (value) {
        ///	<summary>
        ///		Get or set whether to copy color profile data from original JPEG image to thumbnail
        ///	</summary>
        ///	<param name="value" type="Boolean" />
        ///	<returns type="Boolean" />
        /* will be created while initialization */
    ...
    autoReduceDimensions: function (value) {
        ///	<summary>
        ///
        ///	</summary>
        ///	<param name="value" type="Boolean">
    ...
    thumbnailWatermark: function (value) {
        ///	<summary>
        ///		Get or set maximum thumbnail watermark.
        ///	</summary>
    ...
    getThumbnailWatermarkPreview: function (params) {
        ///	<summary>
        ///		Get or set thumbnail watermark preview.
        ///	</summary>
        ///	<param name="params" type="{image:Image,callback:Function}">
        ///	</param>
        /* will be created while initialization */
    },
    thumbnailApplyCrop: function (params) {
        ///	<summary>
        ///
        ///	</summary>
        ///	<param name="value" type="Boolean">
    ...
    thumbnailApplyDrawTool: function (params) {
        ///	<summary>
        ///
        ///	</summary>
        ///	<param name="value" type="Boolean">
    ...
    thumbnailResizeQuality: function (params) {
        ///	<summary>
        ///
        ///	</summary>
        ///	<param name="value" type="string">
    ...
    thumbnailResizeFailureDetector : function(value){
        ///	<summary>
        ///
        ///	</summary>
        ///	<param name="value" type="Object">
    ...
    thumbnailSizeLimit : function(value){
        ///	<summary>
        ///
        ///	</summary>
        ///	<param name="value" type="Object">
    ...
    thumbnailSizeType : function(value){
        ///	<summary>
        ///
        ///	</summary>
        ///	<param name="value" type="Object">
    ...
    thumbnailUseExifPreview : function(value){
        ///	<summary>
        ///
        ///	</summary>
        ///	<param name="value" type="Boolean">
    ...
    thumbnailResizeCmyk : function(value){
        ///	<summary>
        ///
        ///	</summary>
        ///	<param name="value" type="Boolean">
    ...
    toJson: function () {
        ...
        if (this._simpleProperties) {
            for (var i = 0, imax = this._simpleProperties.length; i < imax; i++) {
                if(this._simpleProperties.hasOwnProperty(i)){
    ...
    _validateMode: function (modeString) {
        var rg = /^((.*=)?(SourceFile|Thumbnail|Icon|None|Zip);)*(.*=)?(SourceFile|Thumbnail|Icon|None|Zip);?$/i;
...
AU.converter.reinit = function (converter, control, type) {
    if (type === 'html') {
        converter.mode = function () {
            if (arguments.length > 0) {
                if (arguments[0]) {
                    this._validateMode(arguments[0]);
                    control.setConverterMode(this._index, arguments[0]);
                }
            } else {
                return control.getConverterMode(this._index);
            }
        };
        if (converter._simpleProperties) {
            for (var i = 0, imax = converter._simpleProperties.length; i < imax; i++) {
                ...
                if (converter._simpleProperties.hasOwnProperty(i) && !property.isAttribute) {
                    ...
                    (function (property) {
                        ...
                        setter = getter = property.render || property.name;
            }
                        ...
                        converter[property.name] = function () {
                            if (arguments.length > 0) {
                                if (arguments[0] != null) {
                                    return control[setter](this._index, arguments[0]);
                                }
                            } else
                                return control[getter](this._index);
                        }
                    })(property);
                }
            }
        ...
        converter.mode = function () {
            if (arguments.length > 0) {
                if (arguments[0]) {
                    this._validateMode(arguments[0]);
                    control.callFlash("setConverterMode", this._index, arguments[0]);
                }
            } else {
                return control.callFlash("getConverterMode", this._index);
            }
        };
                ...
                if (converter._simpleProperties.hasOwnProperty(i) && !property.isAttribute) {
                    ...
                    (function (property) {
                        ...
                        converter[property.name] = function () {
                            if (arguments.length > 0) {
                                if (arguments[0] != null) {
                                    return control.callFlash(setter, this._index, arguments[0]);
                                }
                            } else
                                return control.callFlash(getter, this._index);
                        }
...
AU.converters.reinit = function (converters) {
	try {
		...
		var uploader = converters._uploader;
		var type = uploader.type();
		var control = uploader.getControl();

		//save converters added before
		var convertersArr = [], i, cnt;
		for (i = 0, cnt = converters._converters.length; i < cnt; i++) {
			var cv = converters._converters[i];
		...
		converters.get = function (index) {
			...
			if (index < this.count()) {
				if (this._items && this._items[index]) {
					...
					AU.converter.reinit(converter, control, type);
		...
		if (type === 'html') {
			...
			while (control.getConverterCount() > 0) {
				control.removeConverter(0);
			}
			...
			converters.add = function () {
				...
				var index = control.addConverter("*.*=SourceFile");
				if (index >= 0) {
					if (arguments[0]) {
			...
			converters.count = function () {
				return control.getConverterCount();
			};
			...
			// remove converters from control
			while (control.callFlash("getConverterCount") > 0) {
				control.callFlash("removeConverter", 0);
			}
			...
			converters.add = function () {
				...
				///	</summary>
				///	<returns type="$au.converter" added converter />
				var index = control.callFlash("addConverter", "*.*=None");
				if (index >= 0) {
					if (arguments[0]) {
						var conv = this.get(index);
						conv.set(arguments[0]);
			...
			converters.count = function () {
				return control.callFlash("getConverterCount");
			};
			...
			converters.remove = function (index) {
				...
				///	</param>
				// remove from control
				control.callFlash("removeConverter", index);
				//remove from internal array
				this._items.splice(index, 1);
				// re-index items
				for (var i = 0, imax = this._items.length; i < imax; i++) {
...
extend(AU.events.prototype, {
    _eventNames: [undefined /*base event*/, undefined /*base event*/, 'beforeUpload', 'afterUpload', 'beforePackageUpload',
        'afterPackageUpload', 'progress', 'error', 'restrictionFailed', 'addingFiles', 'trace', 'uploadFileCountChange','selectionChange','imageEditorClose','beforeSendRequest'],
...
AU.file.prototype = {
    ...
    file: function () {
        ///	<summary>
        ///		Get file handle in the upload pane.
        ///	</summary>
        ///	<returns type="File|Blob" />
        /* will be created while initialization */
    },
    width: function () {
        ///	<summary>
    ...
    calculateHash: function (callback) {
        ///	<summary>
        ///		callback
        ///	</summary>
        /* will be created while initialization */
...
AU.file.reinit = function (control, obj, type) {

    if (type == 'html') {
        obj.angle = function (angle) {
            if (angle || angle === 0) {
                control.setUploadFileAngle(this._index, angle);
            } else {
                return control.getUploadFileAngle(this._index);
            }
        };
        obj.description = function (description) {
            if (typeof description !== "undefined") {
                control.setUploadFileDescription(this._index, description);
            } else {
                return control.getUploadFileDescription(this._index);
            }
        };
        obj.height = function (callback) {
            return control.getUploadFileHeight(this._index, callback);
        };

        obj.width = function (callback) {
            return control.getUploadFileWidth(this._index, callback);
        };

        obj.exif = function (callback) {
            return control.getUploadFileExif(this._index, callback);
        };

        obj.iptc = function (callback) {
            return control.getUploadFileIptc(this._index, callback);
        };

        obj.modified = function (callback) {
            return control.getUploadFileModifiedDate(this._index);
        };

        obj.type = function (callback) {
            return control.getUploadFileType(this._index);
        };

        obj.colorSpace = function (callback) {
            return control.getUploadFileColorSpace(this._index, callback);
        };

        obj.guid = function () {
            return control.getUploadFileGuid(this._index);
        };
        
        obj.name = function (name) {
            if(typeof name == "undefined"){
                return control.getUploadFileName(this._index);
            }else{
                control.setUploadFileName(this._index, name);
            }     
        };

        obj.relativePath = function (path) {
            if(typeof path == "undefined"){
                return control.getUploadFilePath(this._index);
            }else{
                control.setUploadFilePath(this._index, path);
            }     
        };

        obj.calculateHash = function(callback){
            control.getUploadFileMD5Hash(this._index, callback.bind(this))
        };

        obj.size = function () {
            return control.getUploadFileSize(this._index);
        };

	obj.file = function () {
            return control.getUploadFile(this._index);
        };
        
       
        obj.remove = function () {
            return control.uploadFileRemoveAt(this._index);
        };
        obj.selected = function (selected) {
            if(typeof selected == "undefined"){
		return control.getUploadFileSelectionAt(this._index);
	    }else{
		control.setUploadFileSelectionAt(this._index, selected);
	    }  
        };
        obj.iconCssClass = function () {
            return control.getUploadFileIconCssClassAt(this._index);
        };
        obj.getPreview = function (params) {
            return control.getUploadFilePreviewSrcAt(this._index, params.callback.bind(obj), params.size, params.mode);
        };
	obj.onUpload = function (callback) {
            return control.uploadFileOnUpdateAt(this._index, callback.bind(obj));
        };
        obj.tag = function (value) {
            if(typeof value == "undefined"){
		return control.getUploadFileTagAt(this._index);
	    }else{
		control.setUploadFileTagAt(this._index, value);
	    }  
        };
	obj.showImageEditor = function(x, y){
		control.showImageEditorById(this.guid(), x, y);
	};
        obj.cropBounds = function(value){
		if(typeof value == "undefined"){
			return control.getUploadFileCropBounds(this._index);
	    	}else{  
                    if(arguments.length == 4){
			control.setUploadFileCropBounds(this._index, arguments);
		    }else if(value instanceof Array || value == null){
			control.setUploadFileCropBounds(this._index, value);
		    }	
	    	}  
	};
        obj.getViewport = function(element){
		var editorViewPort = control.getImageEditorViewPortById(this.guid(), element);
		return {
				angle: function(value){
					if(typeof value == "undefined"){
						return editorViewPort.getAngle();
					}else{
						editorViewPort.setAngle(value);
                                                editorViewPort.redraw();
					}
				},  
				applyCrop: function(){
					editorViewPort.crop();
				...
				cropBounds: function(value){
					if(typeof value == "undefined"){
						var cb = editorViewPort.getCropBounds();
						return cb ? cb.toString().split(",").map(function(val){return parseInt(val, 10)}) : null;
					}else{
						editorViewPort.setCropBounds(value);
					}
				}, 
				moveCropBounds: function (x, y) {
					editorViewPort.moveCropBounds(x,y);
				}, 
				resizeCropBounds: function (x, y) {
					editorViewPort.resizeCropBounds(x,y);
				}, 
				reset: function() {
					editorViewPort.reset();
				}, 
				cropBoundsVisible: function(value) {
					if(typeof value == "undefined"){
						return editorViewPort.isCropEnabled();
					}else{
						editorViewPort.setCropEnabled(value);
					}
				} ,
				dispose: function() {
        ...
        obj.angle = function (angle) {
            if (angle || angle === 0) {
                control.callFlash("setUploadFileAngle", this._index, angle);
            } else {
                return control.callFlash("getUploadFileAngle", this._index);
            }
        };
        obj.description = function (description) {
            if (description !== undefined) {
                control.callFlash("setUploadFileDescription", this._index, description);
            } else {
                return control.callFlash("getUploadFileDescription", this._index);
            }
        };
        obj.height = function () {
            return control.callFlash("getUploadFileHeight", this._index);
        };
        obj.name = function () {
            return control.callFlash("getUploadFileName", this._index);
        };
        obj.relativePath = function(){};
        obj.selected = function (selected) {
            if (selected !== undefined) {
                control.callFlash("setUploadFileSelected", this._index, selected);
            } else {
                return control.callFlash("getUploadFileSelected", this._index);
            }

        };
        obj.size = function () {
            return control.callFlash("getUploadFileSize", this._index);
        };
        obj.width = function () {
            return control.callFlash("getUploadFileWidth", this._index);
        };
        obj.remove = function () {
            return control.callFlash("removeAt", this._index, true);
        };

        obj.guid = function () {
            return control.callFlash("getUploadFileGuid", this._index);
        };
    } else {
        throw new Error('Not supported: ' + type);
    }
...
AU.files.prototype = {
    ...
    add: function (name, file) {
        ...
        ///	<param name="file" type="Blob|String"></param>
    }
};
...
AU.files.reinit = function (obj) {
    ...
    if (type === 'html') {
        obj.get = function (index) {
                index = typeof index == 'string' ? control.getUploadFileIndex(index) : index;
        ...
        obj.count = function () {
            return control.getUploadFileCount();
        };

        obj.removeAll = function () {
            return control.uploadFileRemoveAll();
        };

        obj.remove = function (param) {
            return control.uploadFileRemoveAt(param);
        };
        obj.add = function (data, name) {
           control.addUploadFiles(data, name);  
        };
    } else if (type === 'flash') {
        ...
        obj.count = function () {
            return control.callFlash("getUploadFileCount");
        };

        obj.removeAll = function () {
            return control.callFlash("removeAll");
        };

		obj.remove = function (param) {
			var isArray = param && param.constructor && param.constructor === Array;
			if(isArray){
				return control.callFlash("removeItems", param);
			}else{
				return control.callFlash("removeAt", param, true);
		}
    	};
    } else {
        throw new Error('Not supported: ' + type);
...
AU.flashControl.prototype = {
    _simpleProperties: [
        ...
        { name: "ipServiceUrl", getter: 1, setter: 1, renderOnly: true },
        ...
        { name: "maxFileToLoadSize", getter: 1, setter: 1 },
      	...
      	{ name: "actualVersion", getter: 1},
      	{ name: "actualReleaseDate", getter: 1},
        { name: "topPaneVisibility", getter: 1, setter: 1},
        { name: "statusPaneVisibility", getter: 1, setter: 1}
    ],
...
AU.informationBar.prototype = {
    ...
    _simpleProperties: [
      { name: "reconnectionText", getter: 1, setter: 1}
    ],
    _methods: [
...
AU.uploadPane.prototype = {
    _simpleProperties: [
        ...
        { name: "addFolderButtonText", getter: 1, setter: 1 },
        ...
        { name: "visible", getter: 1, setter: 1 ,render: "UploadPaneVisibility" },
        { name: "sortMode", getter: 1, setter: 1 },
        { name: "sortOrder", getter: 1, setter: 1 }
    ],

    getParams: getParams,

    dropFilesHereText: function(text){
    	///	<summary>
        ///		Get or set 'drop files here element' button text
        ///	</summary>
        ///	<param name="value" type="String|Element" />
        ///	<returns type="String|Element" />
    },
    addFolder: function(){
    	if (this._uploader.type() === 'html')
        	this._uploader.getControl().addFiles(true);
    },
    addFolders: function(){
    	this.addFolder();
    },
    addFiles: function(){
    	if (this._uploader.type() === 'html')
        	this._uploader.getControl().addFiles();
    },
    addFilesButtonText: function (value) {
     ...
     addFolderButtonText: function (value) {
        ///	<summary>
        ///		Get or set 'Add Folder Button' button text
        ///	</summary>
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    ...
    sort: function(mode, order){
    	if (this._uploader.type() === 'html')
        	this._uploader.getControl().sort(mode, order);
    },
};
...
AU.messages.prototype = {
	...
	{ name: "fileNameNotAllowed", getter: 1, setter: 1, render: "FileNameNotAllowedMessage" },
        ...
        { name: "dndFolderFileLimitExceeded", getter: 1, setter: 1 },
        { name: "cmykImagesNotAllowed", getter: 1, setter: 1 }
    ],
    dndFolderFileLimitExceeded: function (value) {
        ///	<summary>
        ///		Gets or sets the message for file limit.
        ///	</summary>
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
...
AU.metadata.prototype = {
    ...
    _simpleProperties: [
        ...
        { name: "enableAllStandardFields", getter: 1, setter: 1 },
        { name: "uploadRequestHeaders", getter: 1, setter: 1 }     
    ],
...
AU.metadata.reinit = function (obj) {
    ...
    if (type === 'html') {
        obj.addCustomField = function (name, value, add) {
            if (name) {
                control.addCustomField(name, value, !!add);
            }
        };

        obj.removeCustomField = function (name) {
            if (name != null) {
                control.removeCustomField(name);
            }
        };

        obj.renameStandardField = function (name, newName) {
            if (name != null && newName != null) {
                control.renameStandardField(name, newName);
            }
        };

        obj.enableStandardField = function (name, enable) {
            if(typeof enable == "undefined"){
                 return control.getEnableStandardField(name);
            }else if(name != null){
                 control.setEnableStandardField(name, enable);
            }
        };
        
        obj.resetCustomFields = function () {
           control.resetCustomFields();
        };

        obj.exif = function (str) {
           control.setExtractExif(str);
        };

        if(typeof obj._exif != "undefined"){
           control.setExtractExif(obj._exif);
        }

        obj.iptc = function (str) {
           control.setExtractIptc(str);
        };
        
        if(typeof obj._iptc != "undefined"){
           control.setExtractIptc(obj._iptc);
        }

        obj.valueSeparator = function (str) {
           control.setMetadataValueSeparator(str);
        };

        if(typeof obj._valueSeparator != "undefined"){
           control.setMetadataValueSeparator(obj._valueSeparator);
        }

        } else if (type === 'flash') {
            obj.addCustomField = function (name, value, add) {
                control.callFlash("addCustomField", name, value, !!add);
            };

           obj.removeCustomField = function (name) {
             if (name != null) {
                control.callFlash("removeCustomField", name);
             }
           };
    } else {
        throw new Error('Not supported: ' + type);
...
AU.restrictions.prototype = {
    ...
    deniedFileMask: function (mask) {
        ///	<summary>
        ///		Get or set files denied  filter in open files dialog.    
        ///	</summary>
        ///	<param name="value" type="Array">
        ///		File filters.
        ///	</param>
        ...
        /* will be created while initialization */
        //if (this._uploader.type() === 'html')
        	//this._uploader.getControl().setDeniedFileMask(mask);
...
AU.statusPane.prototype = {
	...
	{ name: "visible", getter: 1, setter: 1, render: "StatusPaneVisibility"},
    ],
...
AU.topPane.prototype = {
    _simpleProperties: [
        ...
        { name: "addFolderHyperlinkText", getter: 1, setter: 1 },
	...
	{ name: "visible", getter: 1, setter: 1 ,render: "TopPaneVisibility" }
    ],
    ...
    addFolderHyperlinkText: function (value) {
        ///	<summary>
        ///		Get or set 'Add ' hyperlink text.
        ///	</summary>
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
...
AU.uploadSettings.prototype = {
    ...
    _simpleProperties: [
        ...
        { name: "enableHeadRequest", getter: 1, setter: 1 }, 
        { name: "chunkSize", getter: 1, setter: 1 },
	{ name: "urlEncodedFilenames", getter: 1, setter: 1, render: "filenamesUrlEncoded" },
        { name: "autoRecoveryMaxAttemptCount", getter: 1, setter: 1},
        { name: "autoRecoveryTimeout", getter: 1, setter: 1},
        { name: "autoRecoveryFilter", getter: 1, setter: 1},
        { name: "connectionTimeout", getter: 1, setter: 1},
        { name: "maxConnectionCount", getter: 1, setter: 1}
			
    ],

    enableInstantUpload: function(isEnabled) {
        ///	<summary>
        ///		Get or set  is instantUpload enabled
        ///	</summary>
        ///	<param name="isEnabled" type="Boolean">
        ///	</param>
        ///	<returns type="Boolean" />
        if (typeof isEnabled != "undefined") {
                this.enableInstantUpload(isEnabled);
             }else{
                return this.enableInstantUpload(); 
            }
      },
    ...
    redirectUrl: function (url) {
        ...
        if (arguments.length > 0) {
            ...
            var self = this;
          	var isRedirectConfigured = false;
		var handlers = this._uploader.events().afterUpload()._handlers;
		for(var i=0;handlers.length;i++){
			if(handlers[i] == self._afterUpload){
				isRedirectConfigured = true;
				break;
			}
		}

		if(!isRedirectConfigured){
                	self._afterUpload = function() {
                    		setTimeout(function () { 
			...
			this._uploader.events().afterUpload().add(self._afterUpload);
		} 
        } else {
...
AU.uploadSettings.init = function (obj, uploader) {
  propertyMaker._typicalInit(obj, uploader);
...
AU.imageEditor = function () {
    ///	<summary>
    ///	dialog for imageEditing
    ///	</summary>
    ///	<returns type="$au.imageEditorDialog" />
};

AU.imageEditor.prototype = {
    _simpleProperties: [
        { name: "enableCrop", getter: 1, setter: 1, render: "ImageEditorEnableCrop" },
        { name: "enableDrawTool", getter: 1, setter: 1, render: "ImageEditorEnableDrawTool" },
        ...
        { name: "drawTool", getter: 1, setter: 1, render: "ImageEditorDrawTool" } ,
        { name: "defaultTool", getter: 1, setter: 1, render: "ImageEditorDefaultTool" },
        { name: "viewport", getter: 1, setter: 1, render: "ImageEditorViewport" }  

     ],
    _methods: [
        { name: "show", controlMethodName: "showImageEditorById" }
    ]
};
...
AU.imageUploaderFlash = function (obj) {
    ...
    ///	</param>
    ///	<returns type="Aurigma.HtmlUploader.imageUploaderFlash" />
...
extend(AU.imageUploaderFlash.fn, {
    _simpleProperties: [
        ...
        { name: "enableAddingFiles", getter: 1, setter: 1 },
        ...
        { name: "enableInstantUpload", getter: 1, setter: 1 },
        { name: "height", getter: 1, setter: 1, defaultValue: "400px", isAttribute: true },
        { name: "licenseKey", getter: 1, setter: 1, renderOnly: true },
        { name: "uploadButtonText", getter: 1, setter: 1 },
        { name: "locale", getter: 1, setter: 1 },
        { name: "width", getter: 1, setter: 1, defaultValue: "600px", isAttribute: true },
	{ name: "folderProcessingMode", getter: 1, setter: 1, defaultValue: "none", isAttribute: true},
	{ name: "enableImageEditor", getter: 1, setter: 1, render: "EnableImageEditor" },
	{ name: "enableExifPreview", getter: 1, setter: 1, render: "EnableExifPreview" },
        { name: "enableDragAndDrop", getter: 1, setter: 1, render: "EnableDragAndDrop" },
        { name: "forceSimpleImageLoad", getter: 1, setter: 1 },
    ...
    reinit: function () {
        ...
        if (this._objectProperties) {
            for (var i = 0, imax = this._objectProperties.length; i < imax; i++) {
                var property = this._objectProperties[i];
                if (this._objectProperties.hasOwnProperty(i) && property.type && typeof property.type.reinit === 'function') {
    ...
    enableAddingFiles: function (value) {
        ///	<summary>
        ///		Enable or disable adding files to upload pane
        ///	</summary>
        ///	<param name="value" type="Boolean" />
        ///	<returns type="Boolean" />
        /* will be created while initialization */
    ...
    version: function () {
      if (this.type() === 'html')
        return this.getControl().getActualVersion();
      else if(this.type() === 'flash')
      	return this.flashControl().actualVersion();
      return "";
    },
    buildDate: function () {
      if (this.type() === 'html')
        return this.getControl().getActualReleaseDate();
      else if(this.type() === 'flash')
      	return this.flashControl().actualReleaseDate();
      return "";
    },
...
var flashRenderer = function (uploader) {
    ...
    var getHtml = function () {
        ...
        function createUploaderParam() {
            ...
            return paramsArr.join("&") + '&controlId=' + uploader.id();
        };
